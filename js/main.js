...
function getSlice(layerNumber) {
  ...
  if (SVGExport) {
    ...
    // svg background
    svg +=
      '<rect x="0" y="0" width="100%" height="100%" style="fill:black; stroke:none"/>\n';
...
$openViewer2D.click(function(e) {
  if (viewer2dWin == null || viewer2dWin.closed) {
    ...
    var opts =
      'menubar=0, toolbar=0, location=0, directories=0, personalbar=0, status=0, resizable=1, dependent=0';

    viewer2dWin = window.open(
      'viewer2d.html',
      'SLAcerViewer2D',
      size + ', ' + opts
    );
...
function updateSlicerSettings() {
  ...
  if ($slicerLayerHeight.val() > 100) {
    $('#slicer-layers-height').addClass('caution');
...
$(
  '#slicer-image-extension-' + (settings.get('slicer.png') ? 'png' : 'svg')
).prop('checked', true);
$('#slicer-make-zip-' + (settings.get('slicer.zip') ? 'yes' : 'no')).prop(
  'checked',
  true
);
$('#slicer-speed-' + (settings.get('slicer.speed') ? 'yes' : 'no')).prop(
  'checked',
  true
);
...
$('#build-volume-unit-' + settings.get('buildVolume.unit')).prop(
  'checked',
  true
);
...
$('#screen-diagonal-unit-' + settings.get('screen.diagonal.unit')).prop(
  'checked',
  true
);
...
function updateTransformValues() {
  ...
  var uniform =
    $('#transform-uniform input[type=radio]:checked').val() == 'yes';
  var input = {
...
$(
  '#transform-mirror-' + (settings.get('transform.mirror') ? 'yes' : 'no')
).prop('checked', true);
